  ...
  update() {
   ...
   if (this.y > canvas.height || this.y < 0) this.speedY *= -1;
    ...
    if (distance < mouse.radius) {
      ...
      const moveX = force * Math.cos(angle) * 5;
      const moveY = force * Math.sin(angle) * 5;
...
function init() {
  ...
  for (let i = 0; i < numberParticles; i++) {
    const size = Math.random() * 1 + 4; // length the pixel
    const x =  Math.random() * canvas.width;
    const y =  Math.random() * canvas.height;
    const speedX = (Math.random() - 0.5) * 2; // horizontal speed
    const speedY = (Math.random() - 0.5) * 2; // vertical speed
